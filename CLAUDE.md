...
venv/bin/python scripts/dev/ratgdo_sim.py --port 8099 --travel 3   # garage-door board simulator; then GARAGE_SIM_URL=http://127.0.0.1:8099 with the line above runs TestGarage.js too
```
...
- `services/garage_monitor.py` — `GarageMonitor`: the garage door through a **ratgdo** board (ESPHome firmware) on the main LAN; myQ has no API. `services/ratgdo_client.py` is the only module that knows the board's HTTP/SSE API (reads the stream with `read1`, never `iter_lines`, which buffers 512 bytes); `services/garage_discovery.py` finds a board among devices the scanner has seen. The ticker thread heartbeats even while disabled (`garage_enabled`); when enabled it holds the board's single SSE stream in a `GarageEvents` helper thread, polls every `garage_poll_interval`, writes `GarageEvent` rows, raises `garage_left_open` / `garage_quiet_hours_open` / `garage_obstruction` / `garage_offline` on the board's `Device` row and resolves them itself, and pushes `garage_status`. Settings keys `garage_*` (runtime only; `garage_password` is never returned by the API). `api/garage.py`; simulator `scripts/dev/ratgdo_sim.py`; `docs/GARAGE_DOOR.md`.
