...
4. Registers 11 API blueprints from `api/*.py` under `/api/<name>` prefixes (set at registration, not in the blueprint): devices, monitoring, config, config-management, analytics, device-control, security, notifications, system, performance, garage. `api_documentation.py` adds `/api/docs`, `/api/redoc` and `/api/openapi.json` (generated from the route map).
5. Instantiates **singleton services** and attaches them to the app: `app._scanner`, `app._monitor`, `app.alert_manager`, `app.bandwidth_monitor`, `app.security_scanner`, `app.configuration_service`, `app.rate_limiter` (Flask-Limiter wrapper; `api/rate_limited_endpoints.py` reads `.rate_limiter.limiter`), `app.performance_monitor`, `app.resource_monitor`, `app.wan_monitor`, `app.garage_monitor`, `app.websocket_optimizer`, `app.websocket_connection_manager`, `app.query_cache`, `app.memory_monitor`, `app.socketio`, `app.security_middleware`, `app.emit_alert_update`, `app.emit_alerts_changed`. Other code reaches them via `current_app.<name>` — preserve the names.
6. Spawns one daemon thread per service in `start_monitoring_services()`. Every loop calls `core.health.record_heartbeat('<ThreadName>')`; the names must match `core/health.py:EXPECTED_THREADS` (`NetworkScanner`, `DeviceMonitor`, `AlertManager`, `BandwidthMonitor`, `PerformanceMonitor`, `ResourceMonitor`, `WanMonitor`, `GarageMonitor`, `SecurityScanner`), which `/api/system/health` reports (503 when any thread is stale; `SecurityScanner` is skipped when scanning is disabled). A thread that is alive but never heartbeated becomes stale after one grace period.
...
- `services/garage_monitor.py` — `GarageMonitor`: the garage door state **read from the Ring "Garage Cam"** (no controller; myQ has no API). `services/ring_client.py` (`RingBridge`: the unofficial `ring-doorbell` library behind one asyncio loop in a daemon thread; sign-in with 2FA, token in `Config.RING_TOKEN_FILE` mode 0600, stored/fresh snapshots, motion history; every error is `RingError`) and `services/door_vision.py` (Pillow change gate + one `anthropic` `messages.parse` call → `DoorReading`; `ANTHROPIC_API_KEY` from `.env`) are the only modules that know those services. The ticker thread heartbeats even while disabled; when enabled it fetches the stored frame every `garage_check_interval`, a fresh one ~45 s after Ring motion or on `POST /api/garage/check`, skips unchanged frames, keeps the previous state below 60 % confidence, writes `GarageEvent` rows (`source='camera'`, `detail` = saved frame under `Config.GARAGE_FRAME_DIR`), raises `garage_left_open` / `garage_quiet_hours_open` / `garage_offline` on a synthetic `ring-cam-<id>` `Device` row and resolves them itself, pushes `garage_status`. Settings keys `garage_*` (runtime only). `api/garage.py`; `scripts/db/v270_garage_cleanup.py` removed the 2.6.0 board rows; `docs/GARAGE_CAM.md`.
...
Server-rendered Jinja + Bootstrap 5 + Chart.js (pinned) + vanilla JS; no build step (templates append `?v={{ app_version }}` to static URLs). `static/js/ui-feedback.js` (loaded by the base template) provides `showToast/showSuccess/showError`, `escapeHtml`, `apiRequest`, `debounce`; `csrf-handler.js` adds the CSRF header to every unsafe fetch — pages must not re-implement any of these (a contract test checks). Escape any string that a LAN device controls (hostnames, service banners) before `innerHTML`. Pages: `/` dashboard, `/smart-home` (garage door + smart-home devices; `static/js/smart-home-page.js`), `/device/<id>`, `/alerts`, `/analytics` (tabs), `/network-map`, `/security`, `/settings`, `/about`. `static/js/device-cards.js` holds the device-card renderer shared by `/` and `/smart-home`. Retired pages redirect. `default_network_range` and `app_version` are injected into every template.
