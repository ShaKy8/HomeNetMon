...
### API layer (`api/`)
Each module exports a Flask `Blueprint` (e.g. `devices_bp`) registered in `app.py`. URL prefix is set at registration, **not** in the blueprint, so a route `@bp.route('/<id>')` inside `api/devices.py` becomes `/api/devices/<id>`. New endpoints should follow the existing pattern: blueprint → caching decorator from `services.query_cache` where appropriate → input validation via `core.validators` → standardized response via `core.error_handler` helpers.

**API response envelope is not uniform** — older endpoints return `{"success": true, "devices": [...]}`-style bodies (legacy), newer ones use the structured `{"error_id", "error_code", "message"}` shape via `core.error_handler`. **Don't migrate one side without the other** — partial migration creates worse confusion than the current state. If you need consistent client-side handling, normalize at the call site (each response either has `"error"` key or doesn't). A unified envelope would be a one-pass breaking change worth doing only if there's an actual third-party consumer; for the in-app JS code there's no payoff.
